...
| `host/catalog.ts` | Live `Cursor.models.list()` for the write box |
...
The UI is a composition layer: routes wire modules, modules own one pane. Login, folder, Send, Stop, Settings, and the chat thread are wired. Settings is a dialog. The model lives in the write box and comes from Cursor’s live catalog for your account; markdown is on by default for Cursor’s replies. Your messages sit on the right; Cursor’s sit on the left.
