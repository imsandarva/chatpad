...
The first useful cut is in: sign in, pick a folder, write, get a streamed reply, and Stop. Markdown can be on or off. The thread is a normal chat, the work — commands, edits, reads — shows in the same turn, and that thread comes back when you open the window.
